<script lang="ts">
  ...
  import MediaShow from "$lib/components/promotions/MediaShow.svelte";
  ...
  import type { Create_Post } from "$lib/services/interface/promotions";
  import { schedulePost } from "$lib/services/NewFacebook/NewFacebookService";
  ...
  let seleted_platform: any[] = [];
  let selectedDate;
  $: {
    const query = $page.url.searchParams;
    selectedDate = query.get("date");
  }
  let create_Post_Info: Create_Post = {
    platform: [],
    media: [],
    fb_message: "",
    insta_message: "",
    dateTime: "",
    link: "www.amazing.com",
  };
  $: {
    selected_library = $library.filter((media: any) => media.selected);
  }
  $: {
    seleted_platform = $platforms_ad.filter((res: any) => res.selected);
  }
  ...
  function formatDate(Select_date: any) {
  ...
  async function handleSubmit() {
    ...
    create_Post_Info.platform = seleted_platform;
    create_Post_Info.media = selected_library;
    create_Post_Info.dateTime = formattedDate;

    const response = await schedulePost(create_Post_Info);
    if (response) {
      create_Post_Info.platform = [];
      create_Post_Info.media = [];
      create_Post_Info.dateTime = new Date();
      create_Post_Info.fb_message = "";
      create_Post_Info.insta_message = "";
    }
  }
...
<div class="p-[16px_17.5px] pb-[80px]">
  <SubHeader />
  <div class="flex lg:flex-row flex-col items-start sm:gap-[16px] gap-3 mt-5">
    <div class="lg:w-[77%] w-full">
      ...
      >
        ...
        <div
          class="bg-blue-bglight text-blue-primary text-sm p-4 rounded-md flex-1 flex justify-between items-end sm:flex-nowrap flex-wrap sm:gap-[32px] gap-[18px]"
        >
          <div class="">
            <div class="font-semibold">Create you own design</div>
            <div class="text-xs">
          ...
          <button
            ...
            type="button"
            style="opacity: 0.5;"
            class="bg-blue-primary text-white py-2 px-3 flex items-center rounded-md"
          >
            <svg
              width="22"
              height="21"
      ...
      >
        <h1 class="flex items-center gap-2.5 text-lg text-font_dark pb-[20px]">
          ...
          <div>
            ...
            <div class="text-blue-primary text-xs">
              <span class="font-bold">0%</span> of text optimized
            </div>
          </div>
        </h1>
        <hr class="bg-gray-200 h-[1px] mb-[20px]" />
        <Accordion class="accordion-main !rounded-[6px]">
          ...
          >
            ...
            >
              <h2 class="text-[#0D64E8] text-[12px] font-medium">
        ...
        <div>
          {#if tab_selected === "facebook"}
            <div>
              ...
              >
              <input
                bind:value={create_Post_Info.fb_message}
                type="text"
          ...
          {#if tab_selected === "instagram"}
            <div>
              ...
              >
              <input
                bind:value={create_Post_Info.insta_message}
                type="text"
                class="border-[#D1D5DB] border-[1px] h-[40px] w-full mt-1 rounded-md"
              />
            </div>
          {/if}
          {#if tab_selected === "Google Ads"}
            <div>
              <label for="" class="text-[#4B5563] text-[14px] w-full block"
                >Post</label
              >
      ...
      >
        <div class="flex items-center justify-end gap-4">
          <button
            on:click={clear}
            type="button"
            class="py-3 w-24 px-5 flex items-center justify-center gap-2 text-sm font-medium text-gray-600 focus:outline-none bg-white rounded-md border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-4 focus:ring-gray-200"
          >
            Cancel
          </button>
          <button
