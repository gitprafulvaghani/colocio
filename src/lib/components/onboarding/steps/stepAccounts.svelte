<script lang="ts">
    ...
    try {
      ...
      if (response.success) {
        if ($socialSignedIn.facebook.data !== null) {
          try {
            $socialSignedIn.facebook.data.entities.forEach((item, index) => {
  ...
  onMount(async () => {
    // TODO: Create a service for the GPT completion. Then run it here on
    // the client side.
    let response = false;
    while (response == false) {
      response = await handleCompletion();
    }
...
<h1 class="m-auto mb-4 text-center text-xl font-semibold">Accounts</h1>
<br />
...
{:else if entities !== null}
  <p class="text-sm font-medium text-gray-900">
    Drag-and-drop the assets and accounts you want to include in your Amazing
    Company profile.
  </p>
  <br /><br />
  <section
    ...
    <div class="flex flex-1 gap-1">
      ...
      <div>
        ...
        <div class="text-xs text-gray-600 sm:text-sm">
          Your profile can only include one asset/account per platform, e.g. 1x
          Facebook page, 1x Google Account. To add more profiles, please upgrade
          or contact us
        </div>
      </div>
    </div>
  ...
  <br />
  <!-- NOTE: This is only here temporally -->
  <strong><h2>{company_infos_temp.name}</h2></strong>
  <form action="">
    <input
      type="text"
      placeholder={company_infos_temp.name}
      bind:value={$company_infos.name}
    />
  </form>
  {#if duplicateItems.length > 0}
    <section class="relative flex gap-8 rounded-md p-4">
      <div class="flex flex-1 gap-1">
        <div>
          <svg
            width="20"
