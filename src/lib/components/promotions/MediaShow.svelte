<script lang="ts">
  ...
  export let SetFbTextStore: writable<string>;
  export let SetinStaTextStore: writable<string>;

  const dispatch = createEventDispatcher();

  export let PreviewData: boolean;
  let currentIndex = 0;
  ...
  if (SetFbTextStore != undefined) {
    SetFbTextStore.subscribe((val) => {
      Fb_details = val;
    });
  }
  if (SetinStaTextStore != undefined) {
    SetinStaTextStore.subscribe((val) => {
      insta_details = val;
    });
  }
...
<div
  class="bg-white rounded-xl drop-shadow-md lg:w-[350px] w-full font-roboto p-4 border border-gray-200"
>
  <div>
    {#if PreviewData == true}
    ...
    <div id="0" class="facebook-post content">
      <div class="flex flex-col py-4 border border-gray-300 rounded-xl">
        ...
        <div class="p-3 text-sm">
          {#if $mainSetTextDetails.description == ""}
            Add description next
          {:else}
            {$mainSetTextDetails.description}
          {/if}
        </div>
        {#if $mainSetTextDetails && $mainSetTextDetails.Selected_Library.length > 0}
          <div
            class="flex flex-nowrap overflow-x-auto overflow-y-hidden gap-2 relative px-4"
          >
            {#each $mainSetTextDetails.Selected_Library as selected_img, index}
    ...
    <div id="1" class="content hidden">
      <div class="flex flex-col pt-4 border border-gray-300 rounded-xl">
        ...
        <div class="p-3 text-sm">
          {#if $mainInstaTextDetails.description == ""}
            Add description next
          {:else}
            {$mainInstaTextDetails.description}
          {/if}
        </div>
        {#if $mainInstaTextDetails && $mainInstaTextDetails.Selected_Library.length > 0}
          <div
            class="flex flex-nowrap overflow-x-auto overflow-y-hidden gap-2 relative px-4"
          >
            {#each $mainInstaTextDetails.Selected_Library as selected_img, index}
              <div class="rounded-lg flex-shrink-0 border border-gray-200">
                <div>
                  <img
                    src={selected_img.src.startsWith("data:")
